---
title: "Inside Consumer and Content Creator Website"
description: "Early concepts for Inside.com's consumer and content creator (CMS) views"
publishDate: "1 Jun 2013"
tags: ["Mahalo", "UI Design", "MVP", "Interaction Design"]
---
![Three mobile devices, showing the article drilldown](./mahalo/insidemobile.png)
This is an early version of Inside.com's consumer and CMS views. Both were designed with speed and efficiency in mind. The CMS was designed to be able to rely fully on keystrokes, no mouse.
![CMS view with a new post open](./mahalo/insidenewpost.png)
The app's objective was to provide a pleasing, visual way to skim the latest breaking news from various sources (Twitter, Facebook, blogs, etc.), and to be able to expand articles when it interested the user. 
![CMS view of an article expanded](./mahalo/insideexpand.png)
Articles were ranked by content creators (power-users and experts) that chose the content in the CMS view. 
![Desktop screenshot of Inside Automotive](./mahalo/insidedesktop.png)
Users could filter content by using main and secondary categories. Categories narrowed so the user can be in the know for all updates on that topic, for example, they could read everything "Inside Automotive."
![Inside user journey](./mahalo/insidediagram.png)
